...
async function fetchCompanies() {
    try {
        ...
        showAlert(appAlert, 'Failed to fetch companies. Please try again later.', 'danger');
    }
}
...
async function initApp() {
    ...
    // Check if user is logged in (optional, if using sessions)
    const sessionId = sessionStorage.getItem('sessionId');
    if (sessionId) {
        // Fetch current user from backend (if using sessions)
...
async function handleRegistration() {
    ...
    try {
        ...
        showAlert(registerAlert, 'An error occurred during registration');
    }
}
...
async function handleLogin() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showAlert(loginAlert, data.error || 'Invalid index number or password');
        }
    } catch (error) {
        console.error('Error during login:', error);
        showAlert(loginAlert, 'An error occurred during login');
    }
}
...
async function requestInterview(companyId) {
    ...
    try {
        // Make API call to request interview
        ...
        if (response.ok) {
            ...
            displayCompanies();
            closeModal();
            showAlert(appAlert, `Interview request with ${data.request.companyName} submitted successfully!`, 'success');
        } else {
            showAlert(appAlert, data.error || 'Failed to submit interview request');
        }
    } catch (error) {
        console.error('Error requesting interview:', error);
        showAlert(appAlert, 'An error occurred while requesting an interview');
    }
}
...
function displayCompanies() {
    ...
    // Loop through the companies and create cards for each
    companies.forEach(company => {
        ...
        card.innerHTML = `
            ...
            <button class="btn-secondary view-company" data-id="${company._id}">View Details</button>
        `;
        ...
        viewButton.addEventListener('click', () => openCompanyModal(company._id));
    });
}
